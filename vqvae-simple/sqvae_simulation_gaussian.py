...
for trial in range(20):
    save_path = "/n/fs/ragr-data/users/yihangs/Celegan/structuredVAE/codes/vqvae-test/vqvae-simple/results/simulation/1d-gaussian/trial_"+str(trial)+"/"
...
print("======== Final Results over 10 trials ========")
print("Latent Z vs True Z: |Pearson r| = %.4f ± %.4f" % (np.mean(abs_r_list), np.std(abs_r_list)))
print("Predicted label vs True label: NMI = %.4f ± %.4f" % (np.mean(predict_label_nmi_list), np.std(predict_label_nmi_list)))
print("Predicted label vs True label: ARI = %.4f ± %.4f" % (np.mean(predict_label_ari_list), np.std(predict_label_ari_list)))
print("GMM fit label vs True label: NMI = %.4f ± %.4f" % (np.mean(gmm_fit_label_nmi_list), np.std(gmm_fit_label_nmi_list)))
print("GMM fit label vs True label: ARI = %.4f ± %.4f" % (np.mean(gmm_fit_label_ari_list), np.std(gmm_fit_label_ari_list)))