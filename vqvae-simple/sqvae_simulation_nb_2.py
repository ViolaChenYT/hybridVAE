...
for trial in range(10):
    ...
    with torch.no_grad():
        ...
        predict_centroid = model.vector_quantization.embedding.data.detach().cpu().numpy()
    ...
    gmm_centroid_sort = np.sort(model_gmm.means_.reshape(-1))
    idx = np.argsort(predict_centroid.reshape(-1)) 
    n = idx.max() + 1
    gmm_centroid_sort_perm = np.empty(n, dtype=gmm_centroid_sort.dtype)      # or np.full(n, fill_value)
    gmm_centroid_sort_perm[idx] = gmm_centroid_sort
    gmm_ave_var = model_gmm.covariances_.reshape(-1).mean()
    model.eval()
    with torch.no_grad():
        model.vector_quantization._reset_codebook(torch.from_numpy(gmm_centroid_sort_perm.reshape(-1,1)).to(device))
        gmm_ave_var_torch = torch.as_tensor(gmm_ave_var, dtype=torch.float32, device=device)
        #var_q_logit_new = model.vector_quantization._compute_var_q_logit(gmm_ave_var_torch)
        #model.vector_quantization.var_q_logit.data.copy_(var_q_logit_new)
    model.train()
    for ep in range(1500, 2000 + 1):
        print(f"epoch: {ep*1}/{2000}")
        model.train()
        m_gather = {}
        idx = 0
        for (xb,) in dl:
            ...
            optimizer.zero_grad()
            if ep<0:
                loss, metrics = model(xb, min(max(ep/200.0, 1.0),10),warmup = 1)
            else:
                loss, metrics = model(xb, min(max(ep/200.0, 1.0),10),warmup = 0)
            #loss, metrics = model(xb, 1.0)
            loss.backward()
            optimizer.step()
            # gather metrics
